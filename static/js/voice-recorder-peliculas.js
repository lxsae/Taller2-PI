...
class VoiceRecorderPeliculas {
    ...
    async verificarEstadoInicial() {
        ...
        this.transcriptionDiv.textContent = "Presiona 'Grabar Audio' y di lo que buscas...";
    ...
    async startRecording() {
        try {
            ...
            if (this.peliculaSeleccionada) {
                this.transcriptionDiv.innerHTML = '<span class="text-blue-400">🎧 Escuchando... di "continuar con comida"</span>';
            } else {
    ...
    async processAudio() {
        try {
            ...
            if (result.success) {
                ...
                this.transcriptionDiv.innerHTML = '<span class="text-red-400">❌ Error en la transcripción</span>';
            }
    ...
    async procesarComandoContinuacion(voiceText) {
        ...
        if (this.containsAny(text, ['comida', 'continuar', 'siguiente', 'seguir', 'avanzar', 'ir a comida'])) {
            this.transcriptionDiv.innerHTML = '<span class="text-green-400">✅ ¡Perfecto! Continuando con comida...</span>';
            ...
            if (sessionOk) {
                ...
                this.transcriptionDiv.innerHTML = '<span class="text-red-400">❌ Error: No hay película seleccionada</span>';
                this.deshabilitarNavegacion();
            }
            
        } else if (this.containsAny(text, ['asientos', 'butacas', 'lugares', 'ir a asientos'])) {
            this.transcriptionDiv.innerHTML = '<span class="text-green-400">✅ Saltando a asientos...</span>';
        ...
        } else if (this.containsAny(text, ['pagar', 'pago', 'comprar', 'finalizar', 'ir a pagos'])) {
            this.transcriptionDiv.innerHTML = '<span class="text-green-400">✅ Yendo al resumen...</span>';
        ...
        } else if (this.containsAny(text, ['cambiar', 'otra', 'nueva', 'buscar otra'])) {
            // Permitir cambiar de película
            await this.limpiarSeleccion();
            this.transcriptionDiv.innerHTML = '<span class="text-blue-400">🔄 Busquemos otra película. Di lo que te gustaría ver.</span>';
    ...
    async getRecommendations(text) {
        try {
            ...
            this.transcriptionDiv.innerHTML = '<span class="text-red-400">❌ Error al buscar películas</span>';
        }
    }
    ...
    showRecommendations(movies) {
        ...
        movies.forEach((movie) => {
            ...
            movieCard.className = "bg-gray-800 rounded-xl p-6 hover:bg-gray-700 transition-all duration-300 cursor-pointer border-2 border-transparent hover:border-blue-500";
            movieCard.addEventListener('click', () => this.seleccionarPelicula(movie));
            ...
            movieCard.innerHTML = `
                <div class="text-center mb-4">
                    ...
                    <p class="text-gray-400 text-sm">${movie.year || 'Año no disponible'}</p>
                </div>
                
                <div class="flex justify-center mb-3">
                    <span class="text-yellow-400 text-lg">${stars}</span>
                    <span class="ml-2 text-gray-400">${movie.rating || 'N/A'}/10</span>
                </div>
                ...
                <div class="text-xs text-gray-400 space-y-1">
                    ...
                    <p><strong>Actores:</strong> ${movie.actors ? movie.actors.split(',').slice(0, 2).join(', ') : 'No disponible'}</p>
                </div>
    ...
    async seleccionarPelicula(movie) {
        try {
            ...
            if (result.success) {
                ...
                this.transcriptionDiv.innerHTML = '<span class="text-red-400">❌ Error al guardar la película</span>';
            }
        } catch (error) {
        ...
        this.moviesList.innerHTML = `
            <div class="col-span-full">
                <div class="bg-green-600/20 border-2 border-green-600 rounded-xl p-6 text-center">
                    ...
                    <p class="text-gray-300 mb-4">${movie.year || ''} • ${movie.genres || ''}</p>
                    <p class="text-gray-400 mb-4">"${movie.plot || 'Descripción no disponible'}"</p>
    ...
    async verificarAntesDeRedirigir(url) {
        try {
            ...
            this.transcriptionDiv.innerHTML = '<span class="text-red-400">❌ Error de conexión. Intenta nuevamente.</span>';
        }
    }
